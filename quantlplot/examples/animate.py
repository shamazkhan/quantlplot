...
def move_view(ax, df):
    ...
    qplt.set_x_pos(df.index[int(x-w)], df.index[int(x+w)], ax=ax)
    anim_counter += 1
...
df = gen_dumb_price()
ax,ax2 = qplt.create_plot('Things move', rows=2, init_zoom_periods=100, maximize=False)
df.plot(kind='candle', ax=ax)
df[['open','close','volume']].plot(kind='volume', ax=ax2)
qplt.timer_callback(lambda: animate(ax, ax2, df), 1/FPS)
qplt.show()
