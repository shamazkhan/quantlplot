...
class quantlplotItem(pg.GraphicsObject):
...
class CandlestickItem(quantlplotItem):
...
class HeatmapItem(quantlplotItem):
...
class ScatterLabelItem(quantlplotItem):
...
def _loadwindata(win):
    try: os.mkdir(os.path.expanduser('~/.quantlplot'))
    except: pass
    try:
        f = os.path.expanduser('~/.quantlplot/'+win.title.replace('/','-')+'.ini')
...
def _start_visual_update(item):
    if isinstance(item, quantlplotItem):
...
# use quantlplot instead of matplotlib
pd.set_option('plotting.backend', 'quantlplot.pdplot')
...
if False: # performance measurement code
    ...
    def timecall(fname, func, *args, **kwargs):
        ...
        return r
    def wrappable(fn, f):
        try:    return callable(f) and str(f.__module__) == 'quantlplot'
        except: return False
    m = sys.modules['quantlplot']
    for fname in dir(m):
